...
def _match_team_name(poly_outcome: str, api_outcomes: list) -> int | None:
    """Polymarket outcome 팀 이름과 Odds API outcome 이름 매칭

    Returns:
        매칭된 api_outcomes의 인덱스, 실패 시 None
    """
...
def _handle_hi_res_capture(
    ...
    if market_type == "moneyline":
        # h2h: 팀 이름으로 Odds API outcome 매칭
        for bm in oracle_data.get("bookmakers", []):
            ...
            for mkt in bm.get("markets", []):
                ...
                if matched_idx is None:
                    print(f"  [HiRes] 팀 매칭 실패: poly={outcome}, "
                          f"api={[o.get('name') for o in api_outcomes]}")
                    break
                ...
                if HI_RES_AVAILABLE:
                    ...
                    imp = 1 / matched_odds if matched_odds > 0 else 0.5
                    imp_other = 1 / other_odds if other_odds > 0 else 0.5
                    total = imp + imp_other
                    fair_matched = imp / total if total > 0 else 0.5
